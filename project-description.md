...
First, a little bit of context. There are a protocol and file format, both called Gemini. It's something between Gopher and a simple HTML/HTTP. And there is a whole vibrant community using it as a web alternative. Basically, people host their blogs just like in the old good days.
...
3. For each .epub file from the queue, send it to a given email address (my Kindle inbox). Move that .epub file to a "processed" folder once email sent successfully.
